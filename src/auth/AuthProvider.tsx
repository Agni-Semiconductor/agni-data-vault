...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<AuthContextValue>(() => ({ session, user: session?.user ?? null, loading, async signInWithOtp(email) { const { error } = await supabase.auth.signInWithOtp({ email, options: { emailRedirectTo: window.location.origin + '/', shouldCreateUser: true } }); return error ? { error: error.message } : {} }, async verifyOtp(email, token) { const { error } = await supabase.auth.verifyOtp({ email, token, type: 'email' }); return error ? { error: error.message } : {} }, async signOut() { await supabase.auth.signOut() } }), [loading, session])
