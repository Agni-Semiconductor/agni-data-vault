...
export default function BenchRunDetail() {
  ...
  if (!dutId) return <section className="space-y-3 rounded-lg border border-border-subtle bg-white p-6"><div className="max-w-xl"><h1 className="text-xl font-semibold">Run cannot be loaded</h1><p className="text-sm text-agni-slate">The link does not identify a DUT. A run ID is only unique together with its DUT ID.</p><Link className="text-agni-orange underline" to="/bench">Back to campaigns</Link></div></section>
  if (detail.isLoading) return <Spinner />
  if (detail.error) return <p className="text-danger">{detail.error.message}</p>
  const run = detail.data?.run
  if (!run) return <section className="space-y-3 rounded-lg border border-border-subtle bg-white p-6"><div className="max-w-xl"><h1 className="text-xl font-semibold">Run not found</h1><p className="text-sm text-agni-slate">No campaign run named {runId} exists for DUT {dutId}.</p><Link className="text-agni-orange underline" to={`/bench?dut_id=${encodeURIComponent(dutId)}`}>Back to campaigns</Link></div></section>
  return <div className="space-y-6">
     <header className="space-y-4"><div className="max-w-3xl"><Link className="text-sm text-agni-orange underline" to={`/bench?dut_id=${encodeURIComponent(dutId)}`}>Back to campaigns</Link><div className="mt-4 flex flex-wrap items-start justify-between gap-3"><div><p className="font-mono text-xs text-agni-slate">{run.dut_id} / {run.run_id}</p><h1 className="mt-1 text-2xl font-semibold">{run.name || run.run_id}</h1><div className="mt-2 flex flex-wrap items-center gap-2"><Badge tone={toneForStatus(run.status)}>{run.status}</Badge><span className="text-sm text-agni-slate">{run.kind ?? 'Unknown kind'}</span><span className="text-sm text-agni-slate">Operator: {run.operator ?? 'Unknown'}</span><span className="text-sm text-agni-slate">Started: {formatDate(run.started_at)}</span></div></div></div>
       <dl className="mt-4 grid max-w-3xl gap-4 rounded-lg border border-border-subtle bg-white p-4 sm:grid-cols-3"><Hash label="Module SHA-256" value={run.module_sha256} /><Hash label="Config SHA-256" value={run.config_sha256} /><Hash label="Safety SHA-256" value={run.safety_sha256} /></dl></div>
    </header>
     <section className="space-y-3"><div><h2 className="text-lg font-semibold">Coverage</h2><p className="text-sm text-agni-slate">Latest attempted state for each visited cell. Untested cells remain blank.</p></div>{coverage.isLoading ? <Spinner /> : coverage.error ? <div className="max-w-xl rounded-lg border border-danger bg-danger-tint p-4"><p className="font-medium text-danger">Coverage unavailable</p><p className="mt-1 text-sm text-danger">{coverage.error.message}</p></div> : coverage.data && coverage.data.cells.length ? <CoverageMap coverage={coverage.data} onCellClick={selectCell} /> : <div className="max-w-xl rounded-lg border border-border-subtle bg-white p-6"><p className="font-medium">No coverage to display</p><p className="mt-1 text-sm text-agni-slate">This run has no attempted cells, so the map is intentionally blank.</p></div>}</section>
    <section ref={cellTable} className="scroll-mt-6 space-y-3 rounded-lg border border-border-subtle bg-white p-4"><h2 className="text-lg font-semibold">Cells</h2><p className="text-sm text-agni-slate">{selectedCell ? `Selected row ${selectedCell.row}, column ${selectedCell.col}. The cell table component will focus this coordinate when integrated.` : 'Select a tested cell in the coverage map to focus it here. The cell table component is owned by the parallel bench-cells workstream.'}</p></section>
      <section className="space-y-3"><h2 className="text-lg font-semibold">Analysis</h2>{detail.isLoading ? <Spinner /> : !detail.data ? <div className="max-w-xl rounded-lg border border-border-subtle bg-white p-6"><p className="font-medium">Analysis unavailable</p><p className="mt-1 text-sm text-agni-slate">The run could not be loaded, so whether an analysis exists is unknown.</p></div> : detail.data.analysis === null ? <div className="max-w-xl rounded-lg border border-border-subtle bg-white p-6"><p className="font-medium">No analysis ingested</p><p className="mt-1 text-sm text-agni-slate">The campaign exists, but no run analysis has been published for it yet. This is a normal ingest state.</p></div> : <div className="max-w-xl rounded-lg border border-border-subtle bg-white p-6"><p className="font-medium">Analysis available</p><p className="mt-1 text-sm text-agni-slate">Extractor {detail.data.analysis.extractor_version} was ingested {formatDate(detail.data.analysis.analysed_at)}. The distributions and per-line rates are below.</p></div>}</section>
