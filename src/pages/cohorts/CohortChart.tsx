...
export function CohortChart({ groups, metric, groupKey }: { groups: CohortGroup[]; metric: MetricDefinition; groupKey: GroupKey }) {
  ...
  return <section className="space-y-3 rounded-lg border border-border-subtle bg-white p-4 shadow-card">
    ...
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{ordered.map((group, index) => { const balanced = isLedgerBalanced(group); return <article key={`${group.group_value}-${index}`} className={`rounded border p-3 text-sm ${balanced ? 'border-border-subtle' : 'border-danger bg-danger-tint'}`}><div className="flex items-baseline justify-between gap-2"><h4 className="font-medium text-agni-ink">{label(group)}</h4><span className="num text-agni-slate">n = {count(group.n_with_metric)}</span></div><p className="mt-1 text-xs text-agni-slate">Members {count(group.n_members)} · no metric row {count(group.n_no_metric_row)} · refused {count(group.n_refused)}</p>{undrawableReason(group, metric.log_scale) && <p className="mt-1 text-xs text-agni-slate">No box drawn: {undrawableReason(group, metric.log_scale)}.</p>}{balanced ? <p className="mt-1 text-xs text-agni-slate">Ledger balances.</p> : <p className="mt-1 text-xs font-medium text-danger">Ledger does not balance: API/database disagreement. Distribution is not trustworthy.</p>}<p className={`mt-2 text-xs ${nonConfirmed(group) ? 'font-medium text-agni-gold' : 'text-agni-slate'}`}>Grouping-key provenance: {provenanceSummaryText(group)}</p></article> })}</div>
  </section>
}
