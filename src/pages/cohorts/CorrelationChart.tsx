...
function drawChart(
  ...
  for (let tick = 0; tick <= 4; tick += 1) {
    const t = yLo + ((yHi - yLo) * tick) / 4,
      y = top + plotH - (plotH * tick) / 4;
    ctx.beginPath();
    ctx.moveTo(left, y);
  ...
  ctx.fillText(
    `${groupKey.label}${groupKey.unit ? ` (${groupKey.unit})` : ""}`,
    left + plotW / 2,
    top + plotH + 34,
  );
  ctx.save();
  ...
  ctx.fillText(
    `${metric.label} (${metric.unit || "unitless"})${log ? ", log" : ""}`,
    0,
    0,
  );
  ctx.restore();
  ...
  if (band.length) {
    ...
    for (let i = band.length - 1; i >= 0; i -= 1)
      ctx.lineTo(px(band[i].x), py(band[i].lo));
    ctx.closePath();
    ctx.fillStyle = "rgba(0,114,178,.12)";
    ctx.fill();
    ctx.beginPath();
    band.forEach((b, i) => {
      const x = px(b.x),
  ...
  for (const point of points) {
    ...
    ctx.beginPath();
    ctx.arc(px(point.x), py(point.y), 2.6, 0, Math.PI * 2);
    ctx.fillStyle =
...
export function CorrelationChart({
  ...
  const half = result.fit
    ? bandHalfWidth(result.fit, result.fit.avg_x)
    : Number.NaN;
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-baseline justify-between gap-x-3">
        <div className="max-w-2xl">
          <h3 className="text-base font-semibold text-agni-ink">
            {metric.label} vs {groupKey.label}
          </h3>
          <p className="text-sm text-agni-slate">
            {result.fit ? (
              <>
                Ordinary least squares on{" "}
                {result.fit_space === "log10_y" ? (
                  <strong>
                    log<sub>10</sub> of {metric.label}
                  </strong>
                ) : (
                  <strong>raw values</strong>
                )}
                , over n = {result.fit.n}. Shaded band is the 95% interval for
                the mean response.
              </>
            ) : (
              <>No fit: {reason}.</>
        ...
        {result.fit && (
          <span className="num text-xs text-agni-slate">
            R² {result.fit.r2.toFixed(3)} · r {r!.toFixed(3)}
          </span>
        )}
      </div>
      ...
      <canvas
        ...
        aria-label={`${metric.label} against ${groupKey.label}, ${result.fit ? `${result.ledger.n_fit} points with a fitted line` : "no fit"}`}
      />

      {result.fit && (
        <p className="max-w-2xl text-sm text-agni-ink">
          Slope {slopeText(result)}
          {Number.isFinite(half) && (
            <span className="text-agni-slate">
              {" "}
              · band half-width {half.toPrecision(2)}{" "}
              {result.fit_space === "log10_y" ? "decades" : result.y_unit || ""}{" "}
              at the mean x, widening towards both ends
            </span>
          )}
        </p>
      )}
      ...
      <p className="max-w-2xl text-xs text-agni-slate">
        x is always fitted raw; only y is transformed, and only when the metric
        is declared log-scale.
        {result.points_sampled && (
          <>
            {" "}
            The scatter shows {result.points_returned.toLocaleString()} of{" "}
            {Number(ledger.n_fit).toLocaleString()} fitted points, thinned
            evenly across the x range —{" "}
            <strong>
              the fit itself used all {Number(ledger.n_fit).toLocaleString()}
            </strong>
            .
          </>
        )}{" "}
        Amber points are measurements whose <em>{groupKey.label}</em> was not
        confirmed.
      </p>
      ...
      <p className="max-w-2xl text-xs text-agni-slate">
        No p-value is reported: a cohort is whatever matched the predicate
        rather than a random sample, so a significance test over it would claim
        more than the data supports. Judge it from n, R² and the slope&rsquo;s
        standard error.
      </p>

      <div
        className={`max-w-2xl rounded border p-3 text-sm ${balanced ? "border-border-subtle" : "border-danger bg-danger-tint"}`}
      >
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <h4 className="font-medium text-agni-ink">
            Every member accounted for
          </h4>
          <span className="num text-xs text-agni-slate">
            {Number(ledger.n_members).toLocaleString()} matched ·{" "}
            {Number(ledger.n_fit).toLocaleString()} fitted
          </span>
        </div>
        {notes.length > 0 ? (
          <ul className="mt-1 list-inside list-disc text-xs text-agni-slate">
            {notes.map((note) => (
              <li key={note}>{note}</li>
            ))}
          </ul>
        ...
        {!balanced && (
          <p className="mt-2 text-xs font-medium text-danger">
            The ledger does not balance: the API and the database disagree about
            this population. Treat the fit as untrustworthy.
          </p>
        )}
      </div>
    </section>
