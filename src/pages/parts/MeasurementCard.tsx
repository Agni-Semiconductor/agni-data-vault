...
function DataPlot({ file }: { file: VaultFile }) { const urlQuery = useQuery({ queryKey: ['file-url', file.id], queryFn: () => getFileUrl(file), staleTime: 240_000 }); const parsed = useParsedFile(urlQuery.data ? { url: urlQuery.data, name: file.original_name, key: file.sha256 ?? file.id } : null); if (urlQuery.isLoading || parsed.isLoading) return <Spinner size="sm" />; if (urlQuery.error || parsed.error) return <p className="px-3 text-xs text-danger">{urlQuery.error?.message ?? parsed.error}</p>; return parsed.parsed ? <QuickPlot parsed={parsed.parsed} compact className="h-full w-full" /> : <p className="text-sm text-agni-slate">No data file</p> }
function ImagePreview({ file }: { file: VaultFile }) { const urlQuery = useQuery({ queryKey: ['file-url', file.id], queryFn: () => getFileUrl(file), staleTime: 240_000 }); if (urlQuery.isLoading) return <Spinner size="sm" />; if (urlQuery.error) return <p className="px-3 text-xs text-danger">{urlQuery.error.message}</p>; return urlQuery.data ? <img src={urlQuery.data} alt={file.original_name} loading="lazy" className="h-full w-full object-contain bg-white" /> : null }
