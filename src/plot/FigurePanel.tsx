...
export function FigurePanel({ panel, title, unit }: { panel: ResolvedPanel; title?: string; unit?: string | null }) {
  ...
  return <section className="space-y-3 rounded-lg border border-border-subtle bg-white p-4 shadow-card">
    ...
    {panel.refusals.length > 0 && <div className="rounded border border-danger bg-white p-3 text-sm"><p className="font-medium text-danger">Refused traces</p><ul className="mt-1 space-y-1 text-agni-slate">{panel.refusals.map((refusal, index) => <li key={`${refusal.label}-${index}`}>{refusalText(refusal)}</li>)}</ul></div>}
  </section>
}
