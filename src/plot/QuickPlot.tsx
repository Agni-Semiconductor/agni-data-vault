...
import { composePlotPng, downloadCanvas, safeFilename } from './exportPng'
...
export function QuickPlot({ parsed, kind: givenKind, height: givenHeight, className, compact = false, title = 'plot' }: { parsed: ParsedFile; kind?: PlotKind; height?: number; className?: string; compact?: boolean; title?: string }) {
  const height = givenHeight ?? (compact ? 176 : 320)
  const [kind, setKind] = useState<PlotKind>(givenKind ?? parsed.detected_kind ?? 'other'); const [x, setX] = useState<string>(); const [y, setY] = useState<string>(); const [y2, setY2] = useState<string>(); const [log, setLog] = useState(Boolean(PROFILES[kind].log_y)); const [hover, setHover] = useState<{ x: number; y: number | null }>(); const [hasChart, setHasChart] = useState(false); const host = useRef<HTMLDivElement>(null); const chart = useRef<uPlot | null>(null)
  ...
  useLayoutEffect(() => {
    ...
    return () => { observer.disconnect(); chart.current?.destroy(); chart.current = null; setHasChart(false) }
  ...
  const exportPng = () => {
    ...
    const chartCanvas = current?.ctx.canvas ?? current?.root.querySelector('canvas')
    if (!current || !chartCanvas) return
    const isLog = log && series.log_y
    const subtitle = isLog ? `|${series.labels.y}| (A), log · ${series.x.length} pts` : `${series.labels.y} vs ${series.labels.x} · ${series.x.length} pts`
    const legend = [{ label: series.labels.y, color: Y_COLOR }, ...(series.y2 ? [{ label: series.labels.y2!, color: Y2_COLOR }] : [])]
    const pxRatio = (current as uPlot & { pxRatio?: number }).pxRatio ?? uPlot.pxRatio
    const canvas = composePlotPng({ chartCanvas, pxRatio, title, subtitle, legend, width: current.width, height: current.height })
    downloadCanvas(canvas, `${safeFilename(title)}.png`)
  }
  return <div className={clsx('space-y-2', className)}>
    {!compact && <div className="mb-2 flex flex-wrap items-center gap-2"><Select label="Kind" value={kind} onChange={(event) => setKind(event.target.value as PlotKind)} options={Object.keys(PROFILES).map((value) => ({ value, label: value }))} /><Select label="X" value={series.labels.x} onChange={(event) => setX(event.target.value)} options={options} /><Select label="Y" value={series.labels.y} onChange={(event) => setY(event.target.value)} options={options} /><Select label="Y2" value={series.labels.y2 ?? ''} onChange={(event) => setY2(event.target.value)} options={y2Options} /><Button type="button" variant="secondary" onClick={() => setLog((value) => !value)}>{log ? 'Log Y' : 'Linear Y'}</Button><Badge tone={series.log_y && log ? 'amber' : 'blue'}>{caption}</Badge>{series.x.length > 5000 && <Badge tone="amber">decimated</Badge>}<Button type="button" size="sm" variant="secondary" className="ml-auto" disabled={!hasChart} onClick={exportPng}>Download PNG</Button></div>}
