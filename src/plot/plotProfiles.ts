...
export function resolveSeries(parsed: ParsedFile, kind: PlotKind, override?: { x?: string; y?: string; y2?: string }): ResolvedSeries {
  ...
  const xLabel = choose(override?.x, profile.x, 0); const yLabel = choose(override?.y, kind === 'dciv' ? [pickDcivChannel(parsed)] : profile.y, 1); const y2Label = override?.y2 ?? (profile.y2 ?? []).map((choice) => parsed.headers[indexOf(parsed.headers, choice)]).find(Boolean)
  ...
  for (const row of parsed.rows) { const xv = number(row[xi]); const yv = number(row[yi]); if (xv === null || yv === null) continue; x.push(xv); const transformed = profile.abs_y ? Math.abs(yv) : yv; y.push(profile.log_y && transformed <= 0 ? null : transformed); if (y2i >= 0) y2.push(number(row[y2i])) }
  return { x, y, ...(y2Label ? { y2 } : {}), labels: { x: xLabel, y: yLabel, ...(y2Label ? { y2: y2Label } : {}) }, log_y: Boolean(profile.log_y) }
}
