...
describe('api core', () => {
  ...
  it('accepts a free-text four-layer stack',async()=>{const stack=[{role:'substrate',material:'Sapphire',t_nm:null},{role:'bottom_metal',material:'HfN',t_nm:100},{role:'fe',material:'AlScN',t_nm:20},{role:'top_metal',material:'Al',t_nm:null}];__setCacheForTests({sample:[...defs.sample,{key:'stack',type:'layer_stack',options_list_key:'materials',column_name:'stack',active:true,required:false,sort_order:5}]},{materials:[{value:'al',active:true}]});expect((await validateEntity('sample',{label:'A',stack})).columns.stack).toEqual(stack);});
  it('rejects invalid stack roles and negative thickness',async()=>{__setCacheForTests({sample:[...defs.sample,{key:'stack',type:'layer_stack',column_name:'stack',active:true,required:false,sort_order:5}]},{});await expect(validateEntity('sample',{label:'A',stack:[{role:'middle',material:'Al',t_nm:1}]})).rejects.toMatchObject({status:422});await expect(validateEntity('sample',{label:'A',stack:[{role:'fe',material:'AlScN',t_nm:-1}]})).rejects.toMatchObject({status:422});});
});
